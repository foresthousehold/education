...
document.addEventListener('DOMContentLoaded', function () {
    ...
    calendar.render();
    const dates = $('#dates').data('dates');
    const replaceDates1 = dates.replace('[', '');
    const replaceDates2 = replaceDates1.replace(']', '');
    const replaceDates3 = replaceDates2.replaceAll(' ', '');
    const splitDates = replaceDates3.split(",");
    $.each(splitDates, function (index, val) {
        var targetDate = "td[data-date = " + "'" + val + "'" + "]";
        $(targetDate).css("background", "#cccccc");
    });
});
